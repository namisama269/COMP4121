...
Base case: j = 0
    ...
    dp1[0][0] = P[0] * Em[0][y[0]] = 2/3 * 3/4 = 1/2
    dp2[i][0] = 0
    dp1[1][0] = P[1] * Em[1][y[0]] = 1/3 * 1/3 = 1/9
    dp2[i][0] = 0

Current iteration: j = 1

    Candidates when i = 0:
    1/10 = dp1[0][0] * Tm[0][0] * Em[0][y[1]] = 1/2 * 4/5 * 1/4
    1/54 = dp1[1][0] * Tm[1][0] * Em[0][y[1]] = 1/9 * 2/3 * 1/4

    The maximum is 1/10 when k = 0

    Candidates when i = 1:
    1/15 = dp1[0][0] * Tm[0][1] * Em[1][y[1]] = 1/2 * 1/5 * 2/3
    2/81 = dp1[1][0] * Tm[1][1] * Em[1][y[1]] = 1/9 * 1/3 * 2/3
...
Current iteration: j = 2
    ...
    Candidates when i = 0:
    1/50 = dp1[0][1] * Tm[0][0] * Em[0][y[2]] = 1/10 * 4/5 * 1/4
    1/90 = dp1[1][1] * Tm[1][0] * Em[0][y[2]] = 1/15 * 2/3 * 1/4

    The maximum is 1/50 when k = 0

    Candidates when i = 1:
    1/75 = dp1[0][1] * Tm[0][1] * Em[1][y[2]] = 1/10 * 1/5 * 2/3
    2/135 = dp1[1][1] * Tm[1][1] * Em[1][y[2]] = 1/15 * 1/3 * 2/3
...
Current iteration: j = 3
    ...
    Candidates when i = 0:
    3/250 = dp1[0][2] * Tm[0][0] * Em[0][y[3]] = 1/50 * 4/5 * 3/4
    1/135 = dp1[1][2] * Tm[1][0] * Em[0][y[3]] = 2/135 * 2/3 * 3/4

    The maximum is 3/250 when k = 0

    Candidates when i = 1:
    1/750 = dp1[0][2] * Tm[0][1] * Em[1][y[3]] = 1/50 * 1/5 * 1/3
    2/1215 = dp1[1][2] * Tm[1][1] * Em[1][y[3]] = 2/135 * 1/3 * 1/3
